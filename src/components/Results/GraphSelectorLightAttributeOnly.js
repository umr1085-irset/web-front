...
class GraphSelectorLightAttributeOnly extends Component {
    constructor(props) {
      super(props);
      this.state = {
        loading:true
      ...
      let tagList = attributes.map((attribute,idx)  => 
        <MDBCol  key={this.props.name+"_2_col1_btn"+idx}>
            <MDBRow middle key={this.props.name+"_2_col1_r1"+idx}>
                    <Button onClick={(e) => this.updateGraph(attribute,'scatter')} id={this.props.name+"_2_btn"+idx} style={{ textTransform: 'none' }}>{attribute}</Button>
             </MDBRow>
...
export default GraphSelectorLightAttributeOnly;
